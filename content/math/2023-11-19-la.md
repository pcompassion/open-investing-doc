...
#### (hidden) existential argument (see below) {#hidden--existential-argument--see-below}
...
The similarity after mapping \\(v\\) onto \\(W\\) by \\( T\\) is the same as the similarity mapping \\( w\\) onto \\(V\\) by \\( T^\* \\). Where \\( Ma(T^\*) = \overline{A^t}\\)
