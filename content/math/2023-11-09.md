...
author = ["littlehome"]
date = 2023-11-14
lastmod = 2023-11-14T19:12:35+09:00
draft = false
+++
...
The function (inside E) is called the link function. It captures our assumption (or model) on how our predictor relates to the probability.
...
So the sum of the remaining terms \\( \mathbb{I}\_{\\{d(x) \neq k\\} Pr(x, Y=k) \\, dx \\) are lowest for the bayes decision. (because it chooses the highest term it could possibly pick)
...
& =E\left[\begin{array}{ccc}
\vdots \\\\
{\left[X\_1-\mu\_1\right.} \\\\
{\left[X\_1-\mu\_1\right.} & \cdots & \left.X\_n-\mu\_n\right]
\end{array}\right] \\\\
...
\\( B ∈ R^{n×n} \\) such that if we define Z = B<sup>-1</sup> (X − µ), then Z ∼ N (0, I).
